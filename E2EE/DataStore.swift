...
class DataStore {
    ...
    var rooms = Dictionary<ConversationID, Array<Message>>()
    ...
    private init() {
        ...
        createMessage(contents: textMsgContent, type: .text, ratioOfIncomingMessage: 0.5)
        createMessage(contents: imagesURL, type: .image, ratioOfIncomingMessage: 0.5)
    }
    
    private func createMessage(contents : Array<String>, type : MsgType, ratioOfIncomingMessage : Float = 0.7){
        var count : Double = 0
        for c in contents{
            count += 1
            ...
            if count / Double(contents.count) >= Double(1 - ratioOfIncomingMessage) {
                ...
                if rooms[msg.conversationID] != nil{
                    ...
                    rooms[msg.conversationID]?.append(msg)
                }else{
                    rooms.updateValue([msg], forKey: msg.conversationID)
                    
                    let conversation = ChatConversation(cvsID: msg.conversationID, membersID: [you.id, sender.id], nameConversation: sender.name, lastMsg: msg)
