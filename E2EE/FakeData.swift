...
class FakeData {
    ...
    private init() {
        ...
        you = User(id: FakeData.randomID(), name: "Anh Trường")
        users = Array()
        messages = Array()
        conversations = Array()
        ...
        for c in nameContent{
            ...
            users.append(User(id: FakeData.randomUserID(), name: c, avatarURL: avatarURL,gender: FakeData.randomGender()))
        }
        
        for c in msgContent{
            let msgID = FakeData.randomMsgID()
            let sender = users[Int.random(in: 0..<users.count)]
            let content = c
            let time = FakeData.randomMsgTime()
            
            // Random message
            var msg : Message
            if Int.random(in: 0...100) % 2 == 0{
                // Text message
                msg = TextMessage(id: msgID, senderId: sender.id, content: content, time: time)
            }else{
                // Image message
                let imageURL = imagesURL[Int.random(in: 0..<imagesURL.count)]
                msg = ImageMessage(id: msgID, senderId: sender.id, contents: [imageURL], time: time)
            }
            ...
            if (!dict.contains(where: { (arg0) -> Bool in
                ...
                let conversation = ChatConversation(cvsID: cvsId,
                                                    ...
                                                    lastMsg: msg,
                                                    muteTime: FakeData.randomMuteTime())
    ...
    private class func randomMsgTime()->MsgTime{
        ...
        if Int.random(in: 0...100) % 2 == 0{
            deliveried = MsgTime.TimeInvalidate
